...
// ================================================================
//  НОРМАЛИЗАЦИЯ КАРТОЧКИ — ✅ с валидацией vote и новыми полями
// ================================================================
function normalizeCard(item, category) {
    ...
    // ✅ Валидация vote_average с защитой от NaN
    var vote_average = 0;
    if (item.vote_average !== undefined && item.vote_average !== null) {
        var parsed = parseFloat(item.vote_average);
        if (!isNaN(parsed)) vote_average = parsed;
    } else if (item.rating && item.rating.kp) {
        var parsedKp = parseFloat(item.rating.kp);
        if (!isNaN(parsedKp)) vote_average = parsedKp;
    } else if (item.rating && item.rating.imdb) {
        var parsedImdb = parseFloat(item.rating.imdb);
        if (!isNaN(parsedImdb)) vote_average = parsedImdb;
    } else if (item.voteCount) {
        var parsedCount = parseFloat(item.voteCount) / 10;
        if (!isNaN(parsedCount)) vote_average = parsedCount;
    }
...
function RutorApiService() {
    ...
    // 🔄 Объединение с данными из плагинов
    ...
    self.list = function (params, onComplete, onError) {
        ...
        var pageSize = Math.min(params.page_size || 25, 25);
