/**
 * Lampa plugin.js (V10) — v3
 *
 * v3 — исправление определения типа карточек (фильм/сериал/шоу) во
 * всех категориях, для российского и зарубежного контента:
 *  - detectMediaMethod(): уже проставленный method/type теперь
 *    доверяется безусловно (а не только когда он === 'tv'), эвристика
 *    (first_air_date и т.п.) используется только как крайний fallback.
 *  - self.full(): при открытии карточки тип больше не пересчитывается
 *    с нуля — берётся уже известный card.method/card.type, что убирает
 *    баг с открытием фильмов как сериалов.
 *  - Добавлен общий forceCardType() — теперь и превью на главном
 *    экране (category()), и полная страница категории (list())
 *    одинаково принудительно проставляют tv-тип для чисто-сериальных
 *    категорий; смешанная категория «Топ 24 часа» не трогается —
 *    её карточки полагаются на тип, определённый сервером.
 */
(function () {
    'use strict';
    ...
    function detectMediaMethod(item) {
        ...
        // 1) Если тип уже явно указан (сервер/воркер это гарантирует
        //    для всех карточек) — доверяем ему безусловно, а не только
        //    когда он равен 'tv'. Раньше проверялось только "=== 'tv'",
        //    и если method был уже верно проставлен как 'movie', код
        //    всё равно проваливался в эвристики ниже и мог перезаписать
        //    его на 'tv' из-за second_air_date/season полей.
        if (item.method === 'tv' || item.type === 'tv') return 'tv';
        if (item.method === 'movie' || item.type === 'movie') return 'movie';

        // 2) Эвристика — только если тип действительно не указан
        //    (например, объект собран вручную где-то ещё в коде).
        if (
    ...
    function RutorApiService() {
        ...
        // ---------- Принудительная простановка типа по категории ----------
        // Категории Lampa жёстко относятся либо к фильмам, либо к
        // сериалам/шоу (см. CATEGORIES выше). Даже если воркер по
        // какой-то причине прислал неточный method/type для отдельной
        // карточки, тип категории — источник истины. Применяется
        // одинаково и к превью на главном экране (category()), и к
        // полной странице категории (list()), чтобы карточки не
        // "перепрыгивали" тип в зависимости от места отображения.
        function forceCardType(meta, cards) {
            // Форсируем тип только для категорий, целиком состоящих из
            // сериалов/шоу. 'Топ 24 часа' — смешанная категория
            // (фильмы + сериалы), поэтому её meta.method='movie' — это
            // служебное значение для Lampa.Activity.push, а НЕ признак
            // того, что все карточки в ней фильмы; там тип каждой
            // карточки уже надёжно определён на сервере (см. worker.js
            // normalize()) и трогать его нельзя.
        ...
        self.list = function (params, onComplete) {
            ...
            self._fetchRaw(
                ...
                function (data) {
                    ...
                    // Принудительно tv-тип для tv-категорий (см. forceCardType)
        ...
        self.full = function (params, onSuccess) {
            ...
            // ВАЖНО: тип карточки уже определён и записан воркером в
            // card.method/card.type на этапе category()/list(). Раньше
            // здесь он безусловно пересчитывался через
            // detectMediaMethod(card), которая могла ошибочно вернуть
            // 'tv' (например, если у карточки оказывалось непустым
            // first_air_date) и подменить корректный 'movie' — из-за
            // этого фильмы открывались как сериалы. Теперь используем
            // уже известный тип и обращаемся к эвристике только если
