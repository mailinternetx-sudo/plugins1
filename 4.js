(function () {
    ...
    function RutorApiService() {
        ...
        self.category = function (params, onSuccess) {
            ...
            CATEGORIES.forEach(function (cat) {
                ...
                self._fetchRaw(url, function (data) {
                    rows.push({ title: cat.title, results: data.results, url: cat.url, source: SOURCE_NAME, total_pages: data.total_pages || 1 });
                    done++;
                    if (done === total) {
        ...
        self.list = function (params, onComplete) {
            ...
            self._fetchRaw(url, function (data) {
                onComplete({ results: data.results, page: data.page, total_pages: data.total_pages, total_results: data.total_results });
        ...
        self.full = function (params, onSuccess) {
            ...
            if (!card.id || card.id <= 0 || String(card.id).length < 3) { fallbackFull({}); return; }
    ...
    function addMenuItem() {
        ...
        var item = $(
            '<li class="menu__item selector" data-action="v10">' +
                '<div class="menu__ico"><svg height="36" viewBox="0 0 24 24" width="36" fill="currentColor"><path d="M12 2L2 8V20H8V14H16V20H22V8L12 2ZM4 10L12 6L20 10V18H17V12H7V18H4V10Z"/><path d="M9 13H15V15H9V13Z"/></svg></div>' +
                '<div class="menu__text">' + SOURCE_NAME + '</div>' +
            '</li>'
        );
        item.on('hover:enter', function () { Lampa.Activity.push({ title: SOURCE_NAME, component: 'category', source: SOURCE_NAME, method: 'category' }); });
        var $after = $('.menu__list [data-action="movie"], .menu__list [data-action="tv"]').first().parent();
        if ($after.length) $after.after(item); else $('.menu__list').append(item);
    }
    ...
    function init() {
        ...
        Lampa.Listener.follow('app', function (e) { if (e.type === 'ready' || e.type === 'render') setTimeout(addMenuItem, 1000); });
